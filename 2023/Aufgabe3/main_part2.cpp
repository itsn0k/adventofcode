    ...
    if (file2.is_open()) {
        ...
        for(int row = 0; row < rowcoutner; row++){ 
            ...
            for(int colume = 0; colume < columecounter; colume++) {
                ...
                if ((input[row][colume] == '*') && ('\0' != input[row][colume] ) && ("" != input[row] ) ) {
                        ...
                        for(int sonecolume = -1; sonecolume < 2; sonecolume++) {
                            ...
                            if (('0' <= input[row+sonerow][colume+sonecolume] && input[row+sonerow][colume+sonecolume] <= '9' ) ) {                     
                                if (!('0' <= input[row+sonerow][colume+sonecolume+1] && input[row+sonerow][colume+sonecolume+1] <= '9' ) || sonecolume == 1 ) {
                                    ...
                                    nummer[counternummer] = GetNummer(input[row+sonerow], (colume+sonecolume));
                                    counternummer++;
                                }
                            }
                        };
                    ...
                    if (counternummer == 2) {
                        std::cout << "debug: "<< counternummer << "||" <<nummer[0] << "*"<< nummer[1] << std::endl;
                        std::cout << "vor rechung: "<< sum << std::endl;
                        sum += nummer[0] * nummer[1];
                        std::cout << "nach rechung: "<< sum << std::endl;
                    }
